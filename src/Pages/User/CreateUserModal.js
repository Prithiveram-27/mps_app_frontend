...
const CreateUserModal = ({
  ...
  useEffect(() => {
    if (currentUser?.date_of_birth) {
      const formattedDate = new Date(currentUser.date_of_birth)
        .toISOString()
        .split("T")[0];
      setDateOfBirth(formattedDate);
    }
    if (currentUser?.marriage_date) {
      const formattedDate = new Date(currentUser.marriage_date)
        .toISOString()
        .split("T")[0];
  ...
  const handleSave = async () => {
    try {
      ...
      if (isUserEdit) {
        ...
        axios
          .post("http://localhost:3000/api/v1/user/createUser", user)
          .then(() => {
            if (isUserEdit) {
              handleUserEdit();
            }
